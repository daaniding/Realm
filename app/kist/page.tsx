...
import RewardReveal from "@/components/kist/RewardReveal";
...
type Phase = "idle" | "tapping" | "opening" | "reveal" | "done";
...
function KistView() {
  ...
  if (!thresholdsRef.current) {
    ...
    let openAt: number;
    if (tierRoll < 0.55) {
      openAt = Math.floor(Math.random() * 2) + 2;
    } else if (tierRoll < 0.8) {
      openAt = Math.floor(Math.random() * 3) + 4;
    } else if (tierRoll < 0.92) {
      openAt = Math.floor(Math.random() * 3) + 6;
    } else {
      openAt = Math.floor(Math.random() * 3) + 8;
    }
    thresholdsRef.current = {
      medium: openAt >= 4 ? Math.floor(openAt * 0.5) : null,
      large: openAt >= 6 ? Math.floor(openAt * 0.7) : null,
      mega: openAt >= 8 ? Math.floor(openAt * 0.85) : null,
      open: openAt,
    };
  }
  ...
  return (
    ...
    >
      ...
      {phase === "reveal" && finalRewards.length > 0 && (
        <RewardReveal
